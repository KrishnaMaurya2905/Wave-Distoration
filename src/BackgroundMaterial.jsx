...
export const BackgroundMaterial = shaderMaterial(
  {
    ...
    u_progress: 0,
    u_aspect: 0,
    u_color: { value: new THREE.Color(0xffffff) }, // Example color value
  },
  `
    varying vec2 vUv;
  ...
  `
    ...
    void main() {
      ...
      float noiseValue = cnoise(vec4(newUv * 40.0 * density, u_time, 1.0));
      float grain = fract(sin(dot(vUv, vec2(12.9898, 78.233) * 2000.0)) * 43758.5453);
      float facets = noiseValue * 2.0;
      float dots = smoothstep(0.1, 0.15, noiseValue);
      float n = facets * dots;
      ...
      vec3 finalColor = vec3(displacement - (n + noiseValue)) - vec3(grain * grainStrength);
